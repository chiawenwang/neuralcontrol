...
class elasticRod
{
	...
	void setDOFBoundaryCondition(double desiredValue, int k);
	
	void updateTimeStep();
	void computeElasticStiffness();
	void prepareForIteration();
	void updateNewtonX(double *dx, double a = 1.0);
	void updateGuess();
	...
	VectorXd getFreeDOF();
	void setFreeDOF(const VectorXd &x_free);

	// MatrixXd computeCurvature(const VectorXd &vertices);
